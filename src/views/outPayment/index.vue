...
<template>
  ...
  <div class="mainContent">
    <div class="regRight">
      ...
      <div class="tableMain">
        <div class="payMain">
          <div class="ksmc">{{params.ksmc}}</div>
          <div class="display">
            <div class="mb30">
              <span class="label">还需个人付款 : </span>
              <span class="money moneys">{{params.ysk}} 元</span>
            </div>
            <div>
              <span class="label labels">医保支付 : </span>
              <span class="money">{{params.qtys}} 元</span>
            </div>
          </div>
          <div class="display">
            <div>
              <span class="label">优惠金额 : </span>
              <span class="money">{{params.jmje}} 元</span>
            </div>
            <div>
              <span class="label labels">本次费用 : </span>
              <span class="money">{{params.zjje}} 元</span>
            </div>
          </div>
...
import {getPayment,getMzxxCreat,getPrePay,getPayResult} from "@/api/index"
export default {
  components: { steps },
  setup () {
    ...
    function payMoney(num){
      ...
      if(num==1){
        let params = {
          jzlsh:data.params.jzlsh,
          selfCfZfblReqList:data.params.selfCfZfblReqList,
          ysk:data.params.ysk
        }
        //预支付生成订单号
        getMzxxCreat(params).then(res=>{
          if(res.errorCode=='SUCCESS'){
            data.out_trade_no = res.data
            let origin = {
              out_trade_no:res.data,
              total_fee: data.params.ysk * 100
            }
            //生成二维码链接
